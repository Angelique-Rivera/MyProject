...
if ($stmt->execute()) {
    $stmt->close();
    $conn->close();
    header("Location: dashboard.php?success=" . urlencode("Student registered successfully."));
    exit;
}

$stmt->close();
$conn->close();
header("Location: dashboard.php?error=" . urlencode("Failed to register student."));
exit;
?>